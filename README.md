...
We have implemented a custom shortcode called `datoPicture` that processes image data provided by DatoCMS. This shortcode generates image tags that can be further processed by the 11ty Image plugin. To use this shortcode in your Nunjucks templates, import and register it in your `.eleventy.js` file:

```js
// .eleventy.js
import datoPicture from "./src/utils/shortcodes/datoPicture.js";

export default function EleventyConfig(config) {
  // ...
  config.addShortcode("datoPicture", datoPicture);
}
```

Then you can use the shortcode in your templates like this:
